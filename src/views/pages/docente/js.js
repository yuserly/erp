...
export default {
    ...
    methods: {
      ...
      formSubmit() {
        ...
        if (!this.$v.$invalid && !this.emailexist) {
          if (this.typeform == "create") {
            ...
            this.axios
              .put(`${this.urlbackend}/docente/editardocente/${this.form.id_docente}`, this.form)
              .then((res) => {
                if (res.data.success) {
      ...
      //   .then((response) => {
      //        this.options = {'nombre' : response.data.nombre+''+response.data.nivel.nombre, 'id_subnivel' : response.data.id_subnivel };
      //   });
      // },
